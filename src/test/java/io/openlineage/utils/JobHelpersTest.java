...
// TODO: skip internal tests unless configured differently
@Tag("internal-test")
public class JobHelpersTest {

  List<RunEvent> prevEvents = new ArrayList<>();
  List<RunEvent> nextEvents = new ArrayList<>();
  Context context = new Context(prevEvents, nextEvents);
  JobHelper helper = new JobHelper(context);
  ...
  void testMergeFacetsCapturesStandardFacets() {
    ...
    prevEvents.add(openLineage.newRunEventBuilder()
      .job(
          openLineage
              .newJobBuilder()
              .name("job1")
    ...
    assertThat(facets.get("documentation").getAdditionalProperties().get("description")).isEqualTo("doc1");
  }
