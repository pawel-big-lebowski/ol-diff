/*
/* Copyright 2018-2025 contributors to the OpenLineage project
/* SPDX-License-Identifier: Apache-2.0
*/

package io.openlineage.utils;

import static org.assertj.core.api.Assertions.assertThat;

import io.openlineage.client.OpenLineage.InputDatasetFacet;
import io.openlineage.client.utils.DatasetIdentifier;
import io.openlineage.utils.Config.IgnoredFacets;
import java.util.List;
import java.util.Optional;
import java.util.Set;
import java.util.stream.Collectors;
import java.util.stream.Stream;
import org.junit.jupiter.api.BeforeAll;
import org.junit.jupiter.api.DisplayName;
import org.junit.jupiter.api.Named;
import org.junit.jupiter.api.Test;
import org.junit.jupiter.params.ParameterizedTest;
import org.junit.jupiter.params.provider.Arguments;
import org.junit.jupiter.params.provider.MethodSource;

@DisplayName("Verify input dataset facets")
public class InputDatasetsDiff {

  private static Context context;
  private static InputDatasetHelper datasetHelper;

  @BeforeAll
  static void setup() {
    context = Context.loadContext();
    datasetHelper = new InputDatasetHelper(context);
  }

  @Test
  @DisplayName("Verify input dataset names and namespaces")
  void verifyInputNames() {
    Set<DatasetIdentifier> prevIdentifiers = context
        .getPrevEvents()
        .stream()
        .flatMap(e -> e.getInputs().stream())
        .map(d -> new DatasetIdentifier(d.getNamespace(), d.getName()))
        .collect(Collectors.toSet());

    Set<DatasetIdentifier> nextIdentifiers = context
        .getNextEvents()
        .stream()
        .flatMap(e -> e.getInputs().stream())
        .map(d -> new DatasetIdentifier(d.getNamespace(), d.getName()))
        .collect(Collectors.toSet());

    assertThat(nextIdentifiers)
        .describedAs("Prev and next should detect the same number of datasets")
        .hasSize(prevIdentifiers.size());
    assertThat(nextIdentifiers)
        .describedAs("Prev and next should have same datasets identified")
        .containsAll(prevIdentifiers);
  }

  @ParameterizedTest
  @MethodSource("prevDatasetFacets")
  @DisplayName("Verify dataset facet: {} {}")
  void verifyDatasetFacet(DatasetIdentifier di, String facetName) {
    if (di == null) {
      assertThat(true)
          .describedAs("No input datasets found")
          .isTrue();
    }
    return;
  }

  @ParameterizedTest
  @MethodSource("prevInputFacets")
  @DisplayName("Verify input dataset facets {}")
  void verifyInputDatasetFacet(DatasetIdentifier di, String facetName) {
    if (di == null) {
      assertThat(true)
          .describedAs("No input datasets found")
          .isTrue();
      return;
    }
    InputDatasetFacet prevFacet = datasetHelper
        .mergedInputDatasetFacets(datasetHelper.prevInputs(di))
        .get(facetName);

    InputDatasetFacet nextFacet = datasetHelper
        .mergedInputDatasetFacets(datasetHelper.nextInputs(di))
        .get(facetName);

    assertThat(nextFacet)
        .overridingErrorMessage("Next facets should contain facet: " + facetName)
        .isNotNull();

    assertThat(nextFacet.getAdditionalProperties())
        .describedAs("Prev input facet additional properties")
        .containsAllEntriesOf(prevFacet.getAdditionalProperties());
  }

  private static Stream<Arguments> prevInputFacets() {
    List<Arguments> args =  datasetHelper
        .prevInputFacets()
        .entrySet()
        .stream()
        .flatMap(entry ->
              entry
                  .getValue()
                  .stream()
                  .filter(facet -> Optional
                      .ofNullable(context.getConfig().getIgnoredFacets())
                      .map(IgnoredFacets::getInputDatasetFacets)
                      .filter(ignoredFacets -> ignoredFacets.contains(facet))
                      .isEmpty()
                  )
                  .map(facetName -> Arguments.of(
                      Named.of("Dataset " + entry.getKey().getName(), entry.getKey()),
                      Named.of("Facet " + facetName, facetName)
                  ))
        )
        .toList();

    if (args.isEmpty()) {
      return Stream.of(Arguments.of(
          Named.of("No input facets to verify", null),
          null
      ));
    } else {
      return args.stream();
    }
  }

  private static Stream<Arguments> prevDatasetFacets() {
    List<Arguments> args =  datasetHelper
        .prevFacets()
        .entrySet()
        .stream()
        .flatMap(entry ->
            entry
                .getValue()
                .stream()
                .filter(facet -> Optional
                    .ofNullable(context.getConfig().getIgnoredFacets())
                    .map(IgnoredFacets::getDatasetFacets)
                    .filter(ignoredFacets -> ignoredFacets.contains(facet))
                    .isEmpty()
                )
                .map(facetName -> Arguments.of(
                    Named.of("Dataset " + entry.getKey().getName(), entry.getKey()),
                    Named.of("Facet " + facetName, facetName)
                ))
        )
        .toList();

    if (args.isEmpty()) {
      return Stream.of(Arguments.of(
          Named.of("No facets to verify", null),
          null
      ));
    } else {
      return args.stream();
    }
  }

  // TODO: modify failure -> to contain dataset with different name
  // TODO: modify failure to contain different schema
  // TODO: modify failure to contain different column level lineage
  // TODO: verify dataset symlinks -> this should be verrified as facets

}
